...
export default function OffersList() {
    ...
    useEffect(() => {
        setLoader(true);
        API.readAll(`/offers?page=${page}`).then(res => {
            setMeta(res.meta);
            if (res.data.length > 0) {
                ...
                res.data.map(item => {
                    ...
                    for (const key in item) {
                        if (key !== "products_array"  && key !== "products" && key !== "ar_image_url"  ) {//item i dont want to list in the table
                            if (key === "image_url") {
                                if (themContext.themeLayout === "admin") {
                                    tempArr["image"] = `<img style="width:50px;" src="${item["image_url"]}" />`;
                                } else {
                                    tempArr["image"] = `<img style="width:50px;" src="${item["ar_image_url"]}" />`;
                                }
                            }
                        ...
                        <CardHeader color={themContext.themeColor}>
                            <Grid container>
                                ...
                                <Grid item xs={4} md={4} style={{ marginTop: 'auto', marginBottom: 'auto', display: 'flex', justifyContent: 'end' }}>
                                    <Link to={`/${themContext.themeLayout}/add-offers`}>
                                        <Button
