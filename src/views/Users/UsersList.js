...
export default function CategoryList() {
    ...
    useEffect(() => {
        setLoader(true);
        API.readAll(`/users?page=${page}&filter[name]=${search}`).then(res => {
            setMeta(res.meta);
            if (res.data.length > 0) {
                ...
                res.data.map(item => {
                    ...
                    for (const key in item) {
                        if (key !== "store") {//item i dont want to list in the table
                            if (key === "image_url") {
                                tempArr["image"] = `<img style="width:50px;" src="${item[key]}" />`;
                            }
                            else {
                            ...
                            <Grid container>
                                <Grid item xs={12} md={12} className="d-md-flex justify-content-between" >
                                    ...
                                    <div className="d-flex">
                                        ...
                                        />
                                        <button className="mx-2 btn btn-secondary" onClick={handleChangeSearch}>Search</button>
                                    </div>
                                </Grid>
                            </Grid>
                        </CardHeader>
