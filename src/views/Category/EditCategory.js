...
export default function EditBrand() {
  ...
  useEffect(() => {
    if (id) {
      setLoader(true)
      API.read(`/categories`, id).then(res => {
        setData(res.data)
        setLoader(true)
      })
    }
  ...
  const onSubmit = (data) => {
    ...
    API.create(`/categories/${id}` , formData).then(res => {
      setLoader(false);
      history.push(`/${themContext.themeLayout}/categories`);
            ...
            <GridItem xs={12} sm={12} md={12}>
              <Card>
                <CardHeader color={themContext.themeColor}>
                  <h4 className={classes.cardTitleWhite}>{t("Edit Category")}</h4>
                </CardHeader>
                <CardBody>
                  <GridContainer>
